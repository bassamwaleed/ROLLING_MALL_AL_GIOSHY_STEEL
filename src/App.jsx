...
export default function App() {
  /* 
    ...
    أي تغيير في هذه المتغيرات سيقوم بتحديث الشاشة تلقائياً
  */
  ...
  const BILLET_WEIGHT = 0.75; // وزن البليت بالطن (لتسهيل التعديل لو اتغير في المصنع)
  ...
  /* 
    ...
    ======================================================
  */
  // محاولة الدخول للسحابة عند فتح التطبيق
  useEffect(() => {
  ...
  // جلب الأطنان والأرشيف من Firebase لحظياً
  useEffect(() => {
    ...
    // onSnapshot: تعني الاستماع اللحظي (أي تغيير في الداتا بيز بيسمع هنا فوراً)
  ...
  */
  const getInitialProductionState = () => {
    ...
    // إذا كانت الساعة قبل 8 صباحاً، اعتبرنا في اليوم السابق
    if (hour < 8) prodDate.setDate(prodDate.getDate() - 1);
    
    const isFriday = prodDate.getDay() === 5; // 5 تعني يوم الجمعة
    let initialShift = '';
  ...
  */
  ...
  // دالة تحدد لون وحالة الستاند بناءً على النسبة المئوية
  ...
  // دالة حفظ البيانات ورفعها للسحابة
  ...
  // تصفير الستاند عند تغيير الدرفيل
  ...
  // تعديل هدف الدرفيل (الماكسيموم) من قِبل المدير
  ...
  // الدالة الأساسية: عندما يضغط الفني على زر الحفظ
  const handleAddShiftProduction = () => {
    const billetsCount = Number(shiftBillets);
    if (billetsCount <= 0 || isNaN(billetsCount)) return; // التأكد أن الرقم سليم
    ...
    setShiftBillets(''); // تفريغ الخانة للفني
  };
  ...
  /* 
    ...
    ======================================================
  */
  // تجميع السجلات في أيام (لتظهر ككروت في الأرشيف)
  ...
  /* 
    ...
    ======================================================
    هذا الجزء يمثل ما يراه المستخدم. يمكنك تعديل الكلاسات (Tailwind) لتغيير التصميم.
  */
  
  // الشاشة الافتتاحية (تسجيل الدخول)
  if (currentUser === 'none') {
    return (
  ...
  return (
    <div className="h-dvh bg-slate-200 text-slate-900 font-sans flex flex-col p-1 gap-1 overflow-hidden" dir="rtl">
      ...
      {/* الشريط العلوي (Navbar) والتنقل بين التبويبات */}
            ...
            {stands.map((stand, idx) => {
              ...
              return (
                <div key={idx} className={`rounded flex flex-col justify-between border ${stand.isActive ? status.bg : 'bg-slate-300 opacity-60'} ${status.border} p-1 relative overflow-hidden`}>
                  ...
                  {/* أزرار التحكم السفلية للستاند (تختلف بين الفني والمدير) */}
                  {currentUser === 'tech' ? (
                ...
                return (
                  <div key={date} className="bg-white border border-slate-200 rounded-lg shadow-sm flex flex-col overflow-hidden">
                    ...
                    <div className="p-1 flex-1 flex flex-col gap-1 bg-white">
                      ...
                      {/* لو اليوم لسه مخلصش وناقص ورديات */}
